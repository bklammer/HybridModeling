function [state] = MuleSim3(state)
%MULESIM3 UVR Hybrid Rocket Motor Model
%   To run a standalone motor simulation, just run the function with no
%   inputs. This will read from the spreadsheet and CEA lookup table
%   specified in the code
%
%   To run design optimization solely on the motor, the input "state"
%   should be a vector containing the design variables, and the function
%   returns the objective function
%
%   For design optimization or uncertainty analysis using "Surrocket," the
%   function accepts and returns a state variable structure

% Benjamin Klammer - 2019
% All calculations performed in metric units

%% INPUTS %%
if nargin == 0 % For use as standalone motor simulator
    input_f = 'MuleSim3INPUT'; % Changes which spreadsheet is used as input
    CEA_f = 'MuleSim3CEA'; % Changes which CEA lookup table is used
    ...
    valid = load('Motor_Validation.mat'); % Experimental data for validation

    % Pre-declare variables before they're poofed into existence by eval
    [del_time, time_max, V_tank, p_tank_init, p_feed, m_ox_tank_init, ...
        n_inj, d_inj, Cd_inj, d_f, rho_f, a, n, L, d_port_init, T_amb, ...
        zeta_d, zeta_cstar, zeta_CF, d_th, p_amb, A_ratio_nozzle, ...
        graph, save] = deal(NaN);
    
    for k = 1:length(input.Symbol) % Read input data from spreadsheet
        eval([input.Symbol{k} '= input.Value(' num2str(k) ');']);     
    end
    C_inj = n_inj * Cd_inj * (pi()*(d_inj/2)^2); % (m^2) Injector effective area
    iteration_tolerance = 0.001;
    state = 0;
    
    
    
elseif (length(state)==6) && isa(state, 'double') % For use as standalone optimization
    graph = 0; % Don't graph
    save = 0; % Don't save
    CEA = load('MuleSim3CEA.mat'); % Combustion input
    N2Osat = load('N2Osat.mat'); % Saturation table for N2O (from NIST)
    
    design = num2cell(state); % Distribute design variable to local variables 
    [V_tank, C_inj, L, d_port_init, d_th, A_ratio_nozzle] = design{:};
    ...
    p_amb = 85600; % (Pa) Pressure at 1400m ASL (Spaceport America)
    ...
    iteration_tolerance = 0.01; % Fraction tolerances on iterative solvers
    
    
    
elseif isstruct(state) % For when being used with "Surrocket.m"
    graph = 0;
    save = 0;
    CEA = state.CEA; % Combustion product lookup table
    N2Osat = state.N2Osat; % Nitrous oxide saturation properties
    
    if isfield(state, 'design') % For design optimization
        design = num2cell(state.design); % Distribute design vector
        [V_tank, C_inj, L, d_port_init, d_th, A_ratio_nozzle] = design{:}; 

        parameters = num2cell(state.parameters); % Distribute parameter vector
        [del_time, time_max, fill_level, p_tank_init, p_feed, rho_f, a, ...
            n, T_amb, p_amb, zeta_d, zeta_cstar, zeta_CF] = parameters{:};
        
    elseif isfield(state, 'parametricstudy') % For parametric study
        design = num2cell(state.parametricstudy); % Distribute design vector
        [V_tank, C_inj, L, d_port_init, d_th, A_ratio_nozzle] = design{:}; 
    ...
    elseif isfield(state, 'uncertainty') % For sensitivity analysis
        uncertainty = num2cell(state.uncertainty);
        [V_tank, fill_level, p_tank_init, C_inj, L, d_port_init, d_th, A_ratio_nozzle, ...
            p_feed, rho_f, a, n, ~, p_amb, T_amb, zeta_d, zeta_cstar, ...
            zeta_CF, ~] = uncertainty{:}; % Distribute uncertainty vector
...
if ~exist('m_ox_tank_init', 'var') % If oxidizer mass is not given, calculate it using fill level
    m_ox_tank_init = V_tank*(rho_liq*fill_level + (1-fill_level)*rho_vap); % Calculate initial oxidizer mass based on fill level
end
...
V_tank_eps = iteration_tolerance*V_tank; % (m^3) Set acceptable tank volume error to 1% of tank volume
u_tank_eps = iteration_tolerance*u_tank; % (m^3) Set acceptable tank specific internal energy error to 1% of initial tank specific internal energy
A_ratio_nozzle_eps = iteration_tolerance*A_ratio_nozzle; % (m^3) Set acceptable nozzle area ratio error to 1% of nozzle area ratio

if exist('d_f', 'var')
    m_f_init = rho_f*L*pi()/4*(d_f^2 - d_port_init^2); % (kg) Calculate initial fuel mass from grain geometry
    m_tot = m_ox_tank_init + m_f_init; % Total mass
else
    m_tot = 0; % If outer diameter not known, add total mass on after simulation
end

% Setting initial conditions
m_ox_tank = m_ox_tank_init; % (kg) Mass of oxidizer in tank
r_cc = d_port_init/2; % (m) Combustion chamber port radius
p_cc = p_amb; % (Pa) Initial Combustion chamber pressure
T_cc = T_amb; % (K) Combustion chamber temperature
T_stag = T_amb; % (K) Combustion chamber stagnation temperature
R_cc = R_const/0.02897; % (J/kg*K) Molar mass of air at 298K
k_cc = 1.4; % Specific heat ratio of air at 298K
Ma = 3; % Initialize the mach number near solution to ensure convergance
burn_time = time_max; % Initialize burn time to max time
...
try % Start try block to catch errors that occur in the main loop (for debugging)
    while 1 % Run until break
        ...
        if x_tank(t) < 1
            ...
            thermo_sat_tank = thermoSat(T_tank(t), 'T', {'p', 'h_liq', ...
                'h_vap', 'rho_liq', 'rho_vap', 'u_liq', 'u_vap'});
            ...
            % Record burn time to be the time at which the tank runs out of oxidizer
            if x_tank(t) > 1
                burn_time = t*del_time; 
            end
        else
        ...
        if isfield(state, 'design') % Combustion chamber pressure constraint for design optimization
            if p_cc(t) > 0.8*(p_tank(t)-p_feed) % Check that the chamber pressure is sufficiently lower than the tank pressure
                error('MuleSim2:highPressure', 'The combustion chamber pressure is too high (p_cc=%7.0f Pa, p_tank=%7.0f Pa, ) \n', p_cc(t), p_tank(t))
            end
        else % Combustion chamber pressure constraint for everything else
            if p_tank(t)-p_feed-p_cc(t) < 0 % Check that the chamber pressure is sufficiently lower than the tank pressure
                error('MuleSim2:highPressure', 'The combustion chamber pressure is too high (p_cc=%7.0f Pa, p_tank=%7.0f Pa, ) \n', p_cc(t), p_tank(t))
            end
        end
        ...
        if isfield(state, 'design') % Mass flux constraint for design optimization
            if G(t) > 500 % Check that the mass flux is not to high (to avoid combustion instability)
                error('MuleSim2:highMassFlux', 'The mass flux is too high (G=%4.0f kg/sm^2) \n', G(t))
            end
        end
        ...
        F_thrust(t) = zeta_CF*(m_dot_cc(t).*vel_exit(t) + (p_exit(t)-p_amb).*A_th.*A_ratio_nozzle_eff); % (N) Rocket Motor Thrust!!!
    ...
    if isa(state, 'double') && (length(state)==6) 
        state = NaN;
        return
    elseif ~isstruct(state) % If used as standalone simulator, output error time
...
m_ox = m_out-m_f; % (kg)
I_tot = trapz(F_thrust)*del_time; % (N*s) Just total impulse during 
v_e = I_tot/m_out; % (m/s)
I_sp = v_e/9.81; % (s)
...
if isstruct(state) % If being used with "Surrocket"
    % Output thrust and mass vs time for trajectory analysis
    TCurve = [I_tot, mean(F_thrust), m_out];
    TCurve = vertcat(TCurve, [time; F_thrust; m_tot]'); 
    
    % Save all pertinent data to the state variable
    state.TC = TCurve;
    state.burn = burn_time/del_time;
    state.mass_prop = m_out;
    state.m_ox_tank_init = m_ox_tank_init;
    state.OF = OF;
    state.d_port_f = d_port_f;
    state.I_sp = I_sp;
    state.A_th = A_th;
    state.AR = A_ratio_nozzle;
    state.Thrust_max = max(F_thrust);
    state.pcc_max = max(p_cc);
    state.meta = [time_sim, time_stamp, computer_name, function_name];
    
elseif isa(state, 'double') && (length(state)==6) % If being used as standalone optimization
    state = 1000 - I_sp;
...
%     e_Fthrust = MAE/mean(X_actual)*100;
    ...
    if save == 1
        saveas(gcf, ['Output\test_', datestr(now, 'yyyy-mm-dd'), '.png']);
    end
    hold off
