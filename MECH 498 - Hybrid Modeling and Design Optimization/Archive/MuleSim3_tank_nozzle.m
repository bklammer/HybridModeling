function [I_tot, T_curve] = MuleSim3_tank_nozzle(CEA_f, input_f)
...
N2Osat = load('../N2Osat.mat'); % Saturation table of N2O
...
m_Al_tank = 0; %9.74; % (kg) tank heat transfer info
C_Al_tank = 900; % (J/kgK)
...
u_ox_tank = (x_tank*u_vap + (1-x_tank)*u_liq); % (J/kg*K) Calculate specific internal energy
U_tank = m_ox_tank_init*u_ox_tank + m_Al_tank*C_Al_tank*T_tank; % (J) Calculate total internal energy

fill_level = (m_ox_tank_init/V_tank - rho_vap)/(rho_liq - rho_vap);
...
u_tank_eps = 0.001*u_ox_tank; % (m^3) Set acceptable tank specific internal energy error to 0.1% of initial tank specific internal energy
...
cp_cc = 1005; % (J/kg*K)
rho_cc = 1.204; %
Ma_sup = 3; % Initialize supersonic mach number to three
Ma_sub = 0.12; % Initialize subsonic mach number to 0.5
Ma = 0.12;

%p_tank = valid.Motor_Validation.UofWBoundlessTankPressure;
...
try % Start try block to catch errors that occur in the main loop (for debugging)
    while 1 % Run until break
        ...
        if x_tank(t) < 1
            ...
            %thermo_sat_tank = thermoSat(p_tank(t), 'p', {'T', 'h_liq', 'h_vap', 'rho_liq', 'rho_vap', 'u_liq', 'u_vap'});
            thermo_sat_tank = num2cell(thermo_sat_tank);
            [p_tank(t), h_liq, h_vap, rho_liq, rho_vap, u_liq, u_vap] = thermo_sat_tank{:};
            U_ox_tank(t) = U_tank(t) - m_Al_tank*C_Al_tank*T_tank(t); % Subtract tank walls to get oxidizer internal energy
            x_tank(t) = (U_ox_tank(t)/m_ox_tank(t) - u_liq)/(u_vap - u_liq);
            u_ox_tank(t) = x_tank(t)*u_vap + (1-x_tank(t))*u_liq;
            ...
            %U_ox_tank(t) = U_tank(t) - *T_tank(t);
            %u_ox_tank(t) = U_ox_tank(t)/m_ox_tank(t);
            uin.rho = rho_tank(t);
            uin.U = U_tank(t);
            uin.m = m_ox_tank(t);
            uin.mc = m_Al_tank*C_Al_tank;
        ...
        if t == 1  % Take average to attenuate numerical instability
        ...
        if t > 1 % First time step is too cold for accurate velocity
        ...
        [T_stag(t+1), rho_cc(t+1), cp_cc(t+1), k_cc(t+1), M_cc(t+1)] = thermo_comb{:};
        T_stag(t+1) = T_stag(t+1)*zeta_cstar^2;
        R_cc(t+1) = R_const/M_cc(t+1);
        
        % NOZZLE AND THRUST CALCULATIONS %
        Ain.k = k_cc(t);
        Ain.A = A_ratio_nozzle;
        if abs(Aerror(Ma_sup(t), Ain)) > A_ratio_nozzle_eps % Nozzle mach number solver
            Ma_sup(t) = secant(@Aerror, Ma_sup(t), Ain); % Supersonic solution
            Ma_sub(t) = secant(@Aerror, Ma_sub(t), Ain); % Subsonic solution
            Ma_shock(t) = sqrt((Ma_sup(t)^2 + 2/(k_cc(t)-1))/(2*k_cc(t)/(k_cc(t)-1)*Ma_sup(t)^2 - 1));
        else
            Ma_sub(t) = Ma_sub(t-1);
            Ma_shock(t) = Ma_shock(t-1);
        end
        
        p_3 = p_stag(t)/(1+(k_cc(t)-1)/2*Ma_sup(t)^2)^(k_cc(t)/(k_cc(t)-1)); % (Pa) 3rd critical presure (optimum expansion)
        p_2 = p_3*(k_cc(t)+1)/(2*k_cc(t)*Ma_shock(t)^2-k_cc(t)+1); %((1+k_cc(t)*Ma_sup(t)^2)/(1+k_cc(t)*Ma_shock(t)^2)); % (Pa) 2nd critical presure (normal shock wave formation)
        p_1 = p_stag(t)/(1+(k_cc(t)-1)/2*Ma_sub(t)^2)^(k_cc(t)/(k_cc(t)-1)); % (Pa) 1st critical pressure (supersonic flow)
        
        if p_atm < p_2 % Full supersonic flow
            Ma(t) = Ma_sup(t);
            p_exit(t) = p_3;
        elseif p_atm < p_1 % Shock waves form in nozzle
            APin.k = k_cc(t);
            APin.A = A_ratio_nozzle;
            APin.p = p_atm/p_stag(t);
            %if abs(APerror(Ma(t), APin)) > A_ratio_nozzle_eps % Shock exit mach number solver
                Ma(t) = secant(@APerror, 0.3, APin);
            %end
            p_exit(t) = p_atm;
%             p_stag_exit = p_atm*(1+(k_cc(t)-1)/2*Ma(t)^2)^(k_cc(t)/(k_cc(t)-1));
%             Pin.k = k_cc(t);
%             Pin.p = p_stag_exit/p_stag(t);
%             M_1 = secant(@Perror, Ma_sup(t), Pin);
%             T_1 = T_stag(t)./(1+(k_cc(t)-1)./2.*M_1^2);
%             p_1 = p_stag(t)/(1+(k_cc(t)-1)/2*M_1^2)^(k_cc(t)/(k_cc(t)-1));
%             T_2 = T_1*((2*k_cc(t)*M_1^2 - k_cc(t) + 1)*((k_cc(t)-1)*M_1^2+2) / ((k_cc(t)+1)^2*M_1^2));
%             p_2 = p_1*(((k_cc(t)+1)*M_1^2) / ((k_cc(t)-1)*M_1^2 + 2));
%             p_stag_2 = p_2*(1+(k_cc(t)-1)/2*M_1^2)^(k_cc(t)/(k_cc(t)-1));
%             T_stag_2 = T_2*(1+(k_cc(t)-1)./2.*Ma(t).^2);
%             T_exit(t) = T_stag_2./(1+(k_cc(t)-1)./2.*Ma(t).^2);
%             p_exit(t) = p_stag_2/(1+(k_cc(t)-1)/2*Ma(t)^2)^(k_cc(t)/(k_cc(t)-1));
        else % Subsonic flow
            Ma(t) = sqrt( 2*((p_stag(t)/p_atm)^((k_cc(t)-1)/k_cc(t))-1)/(k_cc(t)-1) );
            p_exit(t) = p_atm;
        end
        ...
        A_ratio_nozzle_eff = A_ratio_nozzle;
            
%         Ma(t) = Ma_sup(t);
%         p_exit(t) = p_stag(t)./(1+(k_cc(t)-1)./2.*Ma(t).^2).^(k_cc(t)/(k_cc(t)-1));
...
if graph == 1 % Graph if indicated
    ...
    %     axis([0 20 0 6000])
%     saveas(gcf, ['Output\Phoenix1A_ptank_', datestr(now, 'yyyy-mm-dd'), '.png']);
    hold off
    ...
    legend('MuleSim3', 'Test Data')
    %     axis([0 20 0 6000])
    saveas(gcf, ['..\Pictures\Phoenix1A_pcc_', datestr(now, 'yyyy-mm-dd'), '.svg']);
    hold off
...
% SET UP FUNCTIONS TO BE ITERATIVELY SOLVED %
    function V = Verror(T, in) % Finds the difference between the estimated and actual tank volume
        ...
        U = in.U - in.mc*T; % Subract tank energy
        x = (U/in.m - u_l)/(u_v - u_l);
    ...
    function U = uerror(T, in) % Finds the difference between the estimated and actual tank internal energy
        u_ox = (in.U - in.mc*T)/in.m;
        U = (thermoSpanWagner(in.rho, T, {'u'}) + 7.3397e+5) - u_ox;
    end
    ...
    function AP = APerror(M, in) % Finds the difference between the estimated and actual combination of pressure and area ratio
        AP = (1./M)*(2./(in.k+1).*(1+(in.k-1)./2.*M.^2)).^((in.k+1)./(2*in.k-2)) ./ (1+(in.k-1)./2.*M.^2).^(in.k/(in.k-1)) - in.p*in.A;
    end
    
    function P = Perror(M, in) % Finds the difference between the estimated and actual stagnation pressure ratio
        P = (((in.k+1).*M.^2)./(2+(in.k-1).*M.^2)).^(in.k./(in.k-1)) .* ((2.*in.k.*M.^2-in.k+1)./(in.k+1)).^(1/(1-in.k)) - in.p;
    end
end
