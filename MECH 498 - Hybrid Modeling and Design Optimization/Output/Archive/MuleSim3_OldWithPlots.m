...
%     load('Pheonix1A_Thrust_100Hz.mat');
%     if length(Pheonix1A_Thrust) > length(time) % Ensure arrays are same size before plotting
%         time_Pheonix1A_Thrust = time;
%         Pheonix1A_Thrust = Pheonix1A_Thrust(1:length(time));
%     else
%         time_Pheonix1A_Thrust = time(1:length(Pheonix1A_Thrust));
%     end
% 
%     figure
%     hold on
%     plot(time, F_thrust) % (N)
%     plot(time_Pheonix1A_Thrust, Pheonix1A_Thrust);
%     title('Pheonix 1A Hot-Fire Test Thrust')
%     xlabel('Time (s)')
%     ylabel('Thrust (N)')
%     legend('MuleSim3', 'Static Test Data')
%     axis([0 18 0 4000])
...
%     load('Boundless_Thrust_100Hz.mat');
%     if length(Boundless_Thrust) > length(time) % Ensure arrays are same size before plotting
%         time_Boundless_Thrust = time;
%         Boundless_Thrust = Boundless_Thrust(1:length(time));
%     else
%         time_Boundless_Thrust = time(1:length(Boundless_Thrust));
%     end
%     
%     figure
%     hold on
%     plot(time, F_thrust)
% %     plot(time_Boundless_Thrust, Boundless_Thrust)
%     %title('Boundless - University of Washington')
%     title('Oronos Prometheus Test #2')
%     xlabel('Time (s)')
%     ylabel('Thrust (N)')
% %     legend('MuleSim3', 'Static Test Data')
% %     axis([0 20 0 6000])
...
% %     ylabel('Thrust (N)')
% %     axis([0 5 0 900])
% % %     saveas(gcf, ['Output\Mule-1_', datestr(now, 'yyyy-mm-dd'), '.png']);
% %     hold off
% %     
%     [fileName, filePath] = uigetfile('*.mat', 'Please select a MATLAB file for thrust comparison');
%     if isequal(fileName, 0) % If user cancels
%         plot(time, F_thrust)
%         hold off
%     else
%         fullFileName = [filePath, fileName];
%         F_act = load(fullFileName);
%         thrust_field = fieldnames(F_act);
%         F_act = F_act.(thrust_field{1});
%         if length(F_act) > length(time) % Ensure arrays are same size before plotting
%             time_F_act = time;
%             F_act = F_act(1:length(time));
%             F_thrust_error = F_thrust;
%         else
%             time_F_act = time(1:length(F_act));
%             F_thrust_error = F_thrust(1:length(F_act));
%         end
% 
%         plot(time, F_thrust)
%         plot(time_F_act, F_act)
%         legend('MuleSim3', 'Static Test Data')
%         hold off
%     end
%     
